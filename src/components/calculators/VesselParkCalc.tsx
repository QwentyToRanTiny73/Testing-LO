...
export default function VesselParkCalc() {
  ...
  const numField = (label: string, val: string, set: (v: string) => void, hint?: string, step = '0.1') => (
    <div>
      <label className="label">{label}</label>
      <input type="number" className="input" value={val} min={0} step={step} onChange={(e) => set(e.target.value)} />
    ...
    <div className="space-y-6">
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          {numField('Мускат, т', tMuscat, setTMuscat)}
          {numField('Кокур, т', tKokur, setTKokur)}
          {numField('Каберне, т', tCab, setTCab)}
          {numField('Саперави, т', tSap, setTSap)}
        </div>
      </div>
      ...
      <div>
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-4">
          {numField('Выход, л/т', yield_, setYield, 'ориентир 650–750 л/т', '10')}
          {numField('Заполнение бродильной, %', fill, setFill, 'запас на шапку/пену', '1')}
          {numField('Красные на мезге, ×', redFactor, setRedFactor, 'объём мезги к вину', '0.05')}
          {numField('Розе-самотёк, %', rosePct, setRosePct, 'съём с каждого красного', '1')}
          {numField('Разбавление, %', dilPct, setDilPct, 'см. дисклеймер о легальности', '1')}
        </div>
      </div>
